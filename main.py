...
class SSF(torch.nn.Module):
    def __init__(self, adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx, num_hidden, num_proj_hidden, lr, weight_decay, drop_edge_rate_1=0.1, drop_edge_rate_2=0.1, drop_feature_rate_1=0.1, drop_feature_rate_2=0.1, encoder="gcn", sim_coeff=0.5, nclass=1, device="cuda"):
        super(SSF, self).__init__()

        self.device = device

        self.edge_index = convert.from_scipy_sparse_matrix(adj)[0]
        print("inting encoder! ")
    ...
    def fit(self, encoder,epochs):
        ...
        for epoch in range(epochs + 1):
            ...
            if (val_c_loss + val_s_loss) < best_loss:
                # print(f'{epoch} | {val_s_loss:.4f} | {val_c_loss:.4f}')
                best_loss = val_c_loss + val_s_loss
                torch.save(self.state_dict(), f'weights_ssf_{encoder}.pt')

    def predict(self,encoder):
        self.load_state_dict(torch.load(f'weights_ssf_{encoder}.pt'))
        self.eval()
...
if args.model == 'ssf':
    ...
    model = SSF(adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx,num_hidden=args.hidden, num_proj_hidden=args.proj_hidden,lr=args.lr,weight_decay=args.weight_decay,drop_edge_rate_1=0.1, drop_edge_rate_2=0.1, drop_feature_rate_1=0.1, drop_feature_rate_2=0.1, encoder="gcn", sim_coeff=0.5, nclass=1, device="cuda")
else:
    print("invalid model")
...
if args.model == 'ssf':
    ...
    model.fit(args.encoder,args.epochs)
    print("Total time elapsed: {:.4f}s".format(time.time() - t_total))

model.predict(args.encoder)
# # Compute statistics
